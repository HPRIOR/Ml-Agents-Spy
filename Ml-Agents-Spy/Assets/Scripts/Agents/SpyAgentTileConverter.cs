...
namespace Agents
{
    public class SpyAgentTileConverter : IAgentTileConverter
    {
        ...
        private readonly List<IEnvTile> _envTiles;
        private List<(ISpyTile, IEnvTile)> _correspondingTiles;
        ...
        private List<ISpyTile> GetInitialAgentTiles =>
            _envTiles
        ...
        private void GetAdjacentTiles(List<ISpyTile> agentTiles, List<IEnvTile> envTiles)
        {
        ...
        private void GetAgentLocations(Func<(ISpyTile, IEnvTile), bool> envTileHasAttribute)
        {
            if (_correspondingTiles is null)
                throw new NullReferenceException(
                    "Corresponding tuple list of Agent and Env tiles has not been created, run GetAdjecentTiles() first");
            _correspondingTiles.Where(envTileHasAttribute).ToList()
                .ForEach(tileTuple => tileTuple.Item1.OccupiedByAgent = true);
        }
    }
}